...
// Search Snap's user index, then add the first hit by user id.
const hits = await client.friends.search("perdyjamie");
if (hits.length > 0) {
  await client.friends.add(hits[0].userId);
  console.log(`added ${hits[0].username}`);
}

// Subscribe to real-time graph changes.
const unsub = client.friends.onChange((snap) => {
  console.log(`mutuals=${snap.mutuals.length} incoming=${snap.incoming.length}`);
});
