...
Returns matching [`FriendsUser`](/docs/api/interfaces/FriendsUser) records with just `userId` / `username` / `displayName` populated — Snap's search index doesn't return the richer flags. Run the hits through `getUsers()` if you need bitmoji or profile-tier data.
