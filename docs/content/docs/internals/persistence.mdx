...
| `session_snapcap_bearer` | SDK via sandbox `sessionStorage` shim | current SSO bearer string |
| `local_snapcap_self` | SDK via sandbox `localStorage` shim | logged-in user's `{ userId, username, displayName }` JSON |
| `local_uds_uds.e2eeIdentityKey.shared` | Bundle via the `pr()` UDS wrapper (slot 8 of `createMessagingSession`) | wrapped Fidelius identity (RWK-encrypted private key + identityKeyId metadata) — bundle-managed |
| `local_uds_uds.*` (other) | Bundle | other UDS-prefixed bundle state (per-message E2EE temp keys, etc.) |

The Snap bundle's other writes to `localStorage` / `sessionStorage` / `indexedDB` land alongside, prefixed with `local_` / `session_` / `indexdb_` respectively. We don't enumerate them — the bundle owns those entries and the SDK leaves them alone (including across `client.logout()`).
