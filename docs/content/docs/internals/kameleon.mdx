...
Internally, the kameleon Module is booted lazily on the first cold-start `isReady()` call (when no restored cookies are in the DataStore) and cached in a process-wide singleton on the sandbox. Subsequent `SnapcapClient` instances in the same process reuse it for free. The same Module mints different tokens for different users because `finalize(username)` rebinds per call — though see the [single-account-per-VM constraint](/docs/internals/architecture#single-account-per-vm-constraint) for why running multiple accounts in one process isn't actually safe.
