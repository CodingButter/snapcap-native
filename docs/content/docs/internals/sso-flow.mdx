...
The full `_mintTicketFromSSO` from `src/api/auth/sso-ticket.ts`:
...
export async function _mintTicketFromSSO(opts: MintTicketOpts): Promise<MintTicketResult> {
  ...
  await jarFetch(location, {
    method: "GET",
    headers: { accept: "text/html,…", referer: `${SSO_HOST}/` },
  });
...
Bearers are short-lived. We don't know the exact TTL — Snap doesn't document it — but we don't need to. The `src/api/auth/refresh.ts` module drives a bearer refresh via the bundle's own `state.auth.refreshToken` thunk (which internally re-runs kameleon attestation + the SSO redirect):

```ts
// src/api/auth/refresh.ts (paraphrased)
export async function refreshAuthToken(ctx, id) {
  const attestation = await ctx.sandbox.kameleon.finalize(id);
  await authSlice(ctx.sandbox).refreshToken("on_demand", attestation);
}
```

The bundle's `refreshToken` calls `state.auth.refreshToken` internally, minting a fresh SSO ticket from the long-lived `__Host-sc-a-auth-session` cookie and populating its Zustand auth slice. The new bearer is immediately available via `authSlice(sandbox).authToken.token`.

Net effect: callers never see bearer expiration. They write `await client.friends.list()` and the auth dance is invisible.

## When this breaks

Two failure modes worth watching:

1. **Cookie expiration.** `__Host-sc-a-auth-session` has `Max-Age=31536000` (one year). The actual server-side validity may be shorter — sliding window, idle-timeout, who knows. When the cookie is dead, the SSO redirect won't issue a ticket. The retry returns `null`, the original 401 surfaces, and the caller has to re-login from username + password. Call `client.authenticate()` again (with credentials present on the client) to re-run the full login.
