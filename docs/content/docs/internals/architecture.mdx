...
- **Outbound message send + real presence frames.** `client.messaging.on("message", ...)` is wired end-to-end (the bundle session decrypts inbound through Fidelius and emits plaintext via `TypedEventBus`); `setTyping` / `setViewing` resolve correctly but don't yet fire a WS frame, and there's no `messaging.send(...)` yet. Both gated on mapping the bundle session's outbound delegate slots.
- **Per-domain manager surfaces for `presence` / `stories` / `media`.** Stub classes today — the surfaces are designed at migration time, not pre-emptively.
