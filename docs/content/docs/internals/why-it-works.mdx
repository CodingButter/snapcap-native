...
- **Receive message bodies** — the bundle decrypts; `MessagingSession.events.on("message", ...)` fires.
...
- Public `client.onMessage(...)` API (the events emitter exists internally; surfacing it on `SnapcapClient` is the next API addition).
