...
This matters because `installShims()` is first-call-wins, but supporting code (the kameleon boot, the chat bundle loader, `mintFideliusIdentity`, `bringUpMessagingSession`) all call `installShims({})` defensively — and depending on test isolation, multiple Sandbox constructors can run in the same process across tests.
