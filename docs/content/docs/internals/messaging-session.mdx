...
- **Inbound presence delegate.** Outbound `setTyping` / `setViewing` / `setRead` work via the convMgr + presence-slice dual path (see `src/api/messaging.ts`), but the bundle's inbound presence delegate hook still drops typing/viewing/read events from peers — the `messaging.on("typing" | "viewing" | "read", ...)` subscriptions exist but never fire.
