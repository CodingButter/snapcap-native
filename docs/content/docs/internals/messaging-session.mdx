...
For non-E2E paths (login, friends, search, presence, conversation enumeration) the SDK reaches Snap's webpack methods directly through the registry — no session harness needed. For inbound message decrypt the bundle's own `En.createMessagingSession(...)` call still drives Snap's WASM-side state machines; the SDK lazily brings up that session on the first `client.messaging.on(...)` / `setTyping` / `setViewing` call. The bring-up lives in `src/auth/fidelius-decrypt.ts:setupBundleSession` and is wired from `src/api/messaging.ts:#bringUpSession`. See [Fidelius](/docs/internals/fidelius) for the identity side of the story.
...
- **Outbound text / media send.** `client.messaging.send(...)` isn't surfaced yet — the bundle's `getSnapManager()` / send path on the live session needs to be wrapped with the same `setupBundleSession` plumbing that drives inbound. The decrypt half is the harder side; outbound is mostly identifying the right Embind class on the session.
- **Real outbound presence frames.** `setTyping` / `setViewing` resolve after the requested duration but don't yet fire a WS frame to peers — the bundle-side presence delegate (sibling slot of the `messagingDelegate` on `Sess.create`) is still being mapped.
