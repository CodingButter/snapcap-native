...
The session bring-up lives in `src/auth/fidelius-decrypt.ts:setupBundleSession` and is invoked from `src/api/messaging.ts:#bringUpSession`. The phases that touch identity:

1. **`mintFideliusIdentity`** (`src/auth/fidelius-mint.ts`) is called purely to boot the chat-bundle WASM into the standalone realm and expose its `Module` for downstream slots. Its minted-identity return value is discarded; we only need the realm — the bundle does its own minting once `createMessagingSession` runs.
...
|---|---|
| Lazy session bring-up | `src/api/messaging.ts:#bringUpSession` |
| Slot 8 (`e2eeIdentityKeyStore`) UDS-backed store + slot 11 returning `[]` | `src/auth/fidelius-decrypt.ts:setupBundleSession` |
| Standalone-WASM realm boot (mint discarded) | `src/auth/fidelius-mint.ts:getStandaloneChatRealm` |
| GrpcManager factory backing `InitializeWebKey` | `src/auth/fidelius-decrypt.ts:setupBundleSession` |
