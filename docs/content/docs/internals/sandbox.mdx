...
- `src/auth/login.ts:176` — wrapping the WebLogin response payload before handing it to the bundle's protobuf decoder.
- `src/transport/grpc-web.ts` — wrapping every AtlasGw / MessagingCore response body before `decode` / `deserializeBinary`.
- `src/auth/messaging-session.ts:702` — projecting host-realm bytes into the bundle's MessageContent codec (module 79752 export `v`).
- `src/shims/websocket.ts:127` — projecting incoming WebSocket binary frames so `e.data instanceof ArrayBuffer` passes the bundle's check.
...
First call wins. `SnapcapClient`'s constructor calls `installShims({ url, dataStore })` eagerly so the sandbox is seeded with the consumer's DataStore before the kameleon boot or any chat-bundle access — otherwise those flows would call `installShims({})` first and bundle storage writes would land in happy-dom's in-memory defaults instead of the consumer's persistence layer.

This singleton model is also the reason the SDK is currently single-account-per-VM. Two `SnapcapClient` instances pointed at different DataStores in the same process collide on the bundle's `localStorage` namespace. Run one process per account.
