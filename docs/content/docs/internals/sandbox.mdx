...
- `src/shims/fetch.ts` — projecting response body bytes into a sandbox-realm `Uint8Array` so the bundle's protobuf decoders can decode them without a cross-realm `instanceof` failure.
