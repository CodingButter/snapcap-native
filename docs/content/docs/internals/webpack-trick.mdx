...
After this runs, `globalThis.__snapcap_p` is the real `__webpack_require__` — `globalThis` here is the **sandbox-realm** global, not the host's. The bundle is eval'd via `sandbox.runInContext(src)` (see [the sandbox chapter](/docs/internals/sandbox)), so `globalThis.__snapcap_p=p,…` lands on the vm.Context's global. SDK code reads it back via `getSandbox().getGlobal("__snapcap_p")`. Then we call `wreq("58116")` and get the kameleon Module factory. Done.
